...
register_definition() {
    if revision=$(aws ecs register-task-definition --container-definitions "$task_def" --family $family --requires-compatibilities "[\"EC2\", \"FARGATE\"]" --network-mode "awsvpc"  --execution-role-arn "arn:aws:iam::$AWS_ACCOUNT_ID:role/ecsTaskExecutionRole" | $JQ '.taskDefinition.taskDefinitionArn'); then
